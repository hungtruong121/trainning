...
import Style from './MyFolderScreenStyle';
...
import TypeFolderComponent from './TypeFolderComponent';
import {strings} from '../../Locate/I18n';
const {width} = Dimensions.get('window');

class MyFolderScreen extends Component {
  constructor(props) {
    super(props);
  ...
  static getDerivedStateFromProps(nextProps) {
    const {errorCode, profile, folder} = nextProps;
    const data = {
      errorCode,
      profile,
  ...
  handleCreateFolder = async () => {
    ...
    try {
      ...
      this.setState({
        isOpen: true,
        msgNotification: 'Create folder failed.',
      });
    }
  };
  ...
  handleRename = async () => {
    ...
    try {
      ...
      this.setState({
        isOpen: true,
        msgNotification: 'Update folder failed.',
      });
    }
  };
  ...
  handleDeleteFolder = async () => {
    ...
    try {
      ...
      this.setState({
        isOpen: true,
        msgNotification: 'Delete folder failed.',
      });
    }
  };
  ...
  handleAutoDeleteFolder = async () => {
    ...
    try {
      ...
      this.setState({
        isOpen: true,
        msgNotification: 'Setting Autodelete folder failed.',
      });
    }
  };
  ...
  handleSearch = async (keyword) => {
    ...
    try {
      teamService.searchFolder(data).then((response) => {
        if (response.success) {
          const {data} = response;
          folder.listTeamFolderDTO = data.listTeamFolderDTO
            ? data.listTeamFolderDTO
            : [];
          this.setState({
            folder,
          });
      ...
      this.setState({
        isOpen: true,
        msgNotification: 'search folder failed.',
      });
    }
  };
      ...
      <Block flex={false}>
        <Block flex={false} column style={{padding: 10, marginBottom: 10}}>
          <Text center size={13.5}>
            {!isAutoDelete ? 'Rename folder' : 'Auto delete'}
          </Text>
          <Text size={12}>{!isAutoDelete ? 'Name' : 'Date'}</Text>
  ...
  renderListFolder = () => {
    const {navigation} = this.props;
    const {folder, isListView} = this.state;
      ...
      listTeamFolderDTO.forEach((item, index) => {
        ...
        listHtml.push(
          <TypeFolderComponent
            ...
            redirect={() => {
              navigation.navigate(Screens.MY_FOLDER_DETAIL, {
    ...
    return (
      <Block style={Style.view}>
        <Header
          ...
          navigation={navigation}
        />
        <Block flex={false} row spcace="between" style={Style.viewInputSearch}>
          <Block flex={false} width="85%">
            <Input
              placeholderTextColor={Colors.placeholder}
              placeholder="Search"
          ...
          <Block
            ...
            center
            middle
            style={{backgroundColor: Colors.gray, width: '15%'}}>
            <TouchableOpacity
              onPress={() => {
                this.setState({
                  isListView: !isListView,
              ...
              <Image
                source={isListView ? Images.iconGridView : Images.iconListView}
        ...
        <KeyboardAvoidingView
          ...
          <ScrollView style={Style.view} showsVerticalScrollIndicator={false}>
            <Block style={Style.container}>
              ...
              <Block
                flex={false}
                style={isListView ? {} : {flexDirection: 'row'}}>
                <TypeFolderComponent
                  style={{marginRight: '5%'}}
                  isDefault
                  folderName={'POST'}
...
MyFolderScreen.defaultProps = {};

MyFolderScreen.propTypes = {
...
export default connect(mapStateToProps, mapDispatchToProps)(MyFolderScreen);
