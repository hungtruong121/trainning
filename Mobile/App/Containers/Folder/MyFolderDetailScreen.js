...
class MyFolderDetailScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      errorCode: '',
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const data = {errorCode, folderDetail, teamId};
    if (!isEditing) {
      data.folderId = folderId;
      data.listFolderId = [folderId];
    }
  ...
  componentDidMount = async () => {
    ...
    this.fetchData(teamId, folderId);
  };

  fetchData = (teamId, folderId) => {
  ...
  handleRedirectFolder = (item) => {
    let {teamId, listFolderId} = this.state;
    listFolderId.push(item.folderId);
    this.setState({
      isEditing: true,
      folderId: item.folderId,
      listFolderId,
    });
    this.fetchData(teamId, item.folderId);
  };
      ...
      listTeamFolderDTO.forEach((item, index) => {
        listHtml.push(
          <TypeFileComponent
            ...
            redirect={() => this.handleRedirectFolder(item)}
  ...
  handleGoback = () => {
    const {listFolderId, teamId} = this.state;
    const {navigation} = this.props;
    if (listFolderId.length === 1) {
      navigation.goBack();
    } else {
      listFolderId.pop();
      this.setState({
      ...
      this.fetchData(teamId, listFolderId[listFolderId.length - 1]);
    }
  };
  ...
  render() {
    ...
    const folderName =
      ...
      folderDetail.pathFolder &&
      folderDetail.pathFolder.length > 0 &&
      folderDetail.pathFolder[0] &&
      folderDetail.pathFolder[0].folderName
        ? folderDetail.pathFolder[0].folderName
        : '';
...
MyFolderDetailScreen.defaultProps = {};

MyFolderDetailScreen.propTypes = {
